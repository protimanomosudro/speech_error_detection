# Inference code for Whispering LLAMA
import json
import os
import sys
import time
...
parser.add_argument('--root', type=str, help='/home/user/Documents/NEU_SFU/speech_error_classification/Whispering-LLaMA/model/adapter_checkpoints')
parser.add_argument('--save_dir', default= '/home/user/Documents/NEU_SFU/speech_error_classification/transcript_outputs/wers',type=str, help='directory to save predictions as JSON files')
...
dtype = torch.float16 if fabric.device.type == "cuda" else torch.float32
# Loading the model
...
if pretrained_path.is_file():
    ...
    # Directory of shards like alpaca_a.pth, alpaca_b.pth ...
    shards = sorted(pretrained_path.glob("*.pth"))
    assert len(shards) > 0, f"No checkpoint shards found in {pretrained_path}"
    for shard in shards:
        print(f"Loading shard {shard}")
        with lazy_load(shard) as checkpoint:
            model.load_state_dict(checkpoint, strict=False)
else:
    raise FileNotFoundError(f"{pretrained_path} is neither a file nor a directory")
...
(_, w_ck_pt) = whisper.load_model("tiny",device='cpu')
...
python3 /home/user/Documents/NEU_SFU/speech_error_classification/Whispering-LLaMA/llama_whisper_adapter_inference.py \
    --pretrained_path 'model/alpaca_a.pth model/alpaca_b.pth model/alpaca_c.pth' \
    --tokenizer_path 'model/alpaca_a.pth model/alpaca_b.pth model/alpaca_c.pth' \
    --data '/home/user/Documents/NEU_SFU/speech_error_classification/Whispering-LLaMA/audio_features/gs_inferences/ac048_2007-08-06_trimmed_train.pt' \
