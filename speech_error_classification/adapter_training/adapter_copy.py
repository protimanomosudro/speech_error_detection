...
parser.add_argument('--data', type=str,help='ac,tft') 
...
devices = args.d
batch_size = 4 / devices # trained atis with 32BS 1 gpu == 64BS with 2 GPUs
micro_batch_size = 2 # was 6 with 500
...
#     "atis": 30,
#     "slurp": 45}
# num_demonstrations = demo_config[dataset]
...
#print(type(train_data))        # list? dict? something else?
#print(list(train_data.keys()) if isinstance(train_data, dict) else None)
...
#max_input_length = 1000 # 800 for v100
max_seq_length = 512  # see scripts/prepare_alpaca.py # 832 for v100
max_input_length = 400 # 800 for v100
...
log_interval = 1
run_name = f"FINAL_Mask_{dataset}_DPP_adapter{learning_rate}_8BSZ" # added z at end to distinguish
...
def main():
    ...
    pretrained_path: str = "/home/user/Documents/NEU_SFU/speech_error_classification/Whispering-LLaMA/model/lit-llama.pth"
...
def train(
    ...
    for iter_num in range(max_iters):
        ...
        # if step_count < warmup_steps: #was <=
        #     # linear warmup
...
def get_batch(fabric: L.Fabric, data: list):
    ...
    if len(data) < micro_batch_size:
        raise ValueError(f"Data list has only {len(data)} elements, but micro_batch_size is {micro_batch_size}")
    
    ix = torch.randint(len(data), (micro_batch_size,)).tolist()
    print(f"Selected indices: {ix}, Data length: {len(data)}")  # Debug info
    
    #ix = torch.randint(len(data), (micro_batch_size,))
    #ix = torch.randint(len(data), (micro_batch_size,)).tolist()
    #ix = ix.tolist()  # convert tensor to Python list of ints
    ...
    for i in ix:
        if i < len(data):
            ...
            if "input_ids" not in data[i]:
                ...
                print(f"Available keys in data[{i}]: {list(data[i].keys())}")
                continue  # Skip this element or handle appropriately
            
            if "labels_with_masked_input" not in data[i]:
                print(f"Warning: 'labels_with_masked_input' key missing in data[{i}]")
                print(f"Available keys in data[{i}]: {list(data[i].keys())}")
                continue  # Skip this element or handle appropriately
            
            input_ids.append(data[i]["input_ids"][:max_input_length].type(torch.int64))
            labels.append(data[i]["labels_with_masked_input"][:max_input_length].type(torch.int64))
        else:
            print(f"Warning: Index {i} out of bounds for data length {len(data)}")
    
    # Check if we have any valid data
    if not input_ids:
        raise ValueError("No valid data found in the selected batch")
...
for i in range(N):
    example = {k: v[i] for k, v in train_data.items()}
    data_list.append(example)
    
def get_batch(fabric: L.Fabric, data: dict):
    #keys = list(data.keys())

    ix = torch.randint(len(keys), (micro_batch_size,)).tolist()
    #ix = ix.tolist()  # convert tensor to Python list of ints
    input_ids = [data[i]["input_ids"][:max_input_length].type(torch.int64) for i in ix]
    labels = [data[i]["labels_with_masked_input"][:max_input_length].type(torch.int64) for i in ix]
